...
/**
 * @author		Antoine
 * based of extension tx_directmail create by (Kasper Skaarhoj, Jan-Erik Revsbech, Stanislas Rolland, Ivan-Dharma Kartolo)
 *
 * @package 	TYPO3
 * @subpackage	tx_dm_newsletter
 */
 ...
 *
 *
 *   89: class tx_directmail_dmail extends t3lib_SCbase
 *  127:     function init()
 *  188:     function printContent()
 *  198:     function main()
 *  354:     function createDMail()
 *  448:     function moduleContent()
 *  470:     function createDMail_quick($indata)
 *  571:     function showSteps($step, $stepTotal = 5)
 *  585:    @ function mailModule_main()
 *  841:     function JSbottom($formname='forms[0]')
 *  879:     function cmd_finalmail($row)
 *  953:     function cmd_send_mail($row)
 * 1080:     function sendTestMailToTable($idLists,$table,$htmlmail)
 * 1108:     function cmd_testmail($row)
 * 1182:     function cmd_displayMailGroup_test($result)
 * 1203:     function getRecordList($listArr,$table,$dim=0,$editLinkFlag=1)
 * 1216:     function cmd_compileMailGroup($group_uid)
 * 1376:     function update_specialQuery($mailGroup)
 * 1431:     @ function createReceipientList()
 * 1472:     @ function createCategoriesList($fields, $table, $whereClause, $press, $order, $style)
 * 1503:     @ function createCategoriesForm()
 * 1534:     function makeCategoriesForm()
 * 1626:     function makeFormInternal($boxID,$totalBox)
 * 1648:     function makeFormExternal($boxID,$totalBox)
 * 1683:     function makeFormQuickMail($boxID,$totalBox)
 * 1706:     function makeListDMail($boxID,$totalBox)
 * 1759:     function cmd_quickmail()
 * 1784:     function cmd_news()
 * 1827:     function linkDMail_record($str,$uid)
 * 1837:     function setURLs($row)
 * 1877:     function getPageCharSet($pageId)
 * 1895:     function addUserPass($url)
 * 1912:     function cmd_fetch($row,$embed=FALSE)
 * 2022:     function directMail_defaultView($row)
 *
 *  @ <- new or modified function, the other function are the same that direct_mail
 *
 * TOTAL FUNCTIONS: 33
 *
 */
...
/**
 * Direct mail Newsletter Module of the tx_dm_newslettre extension for sending newsletter
 *
 */
class  tx_dm_newsletter_mod1 extends t3lib_SCbase {
...
//		$this->include_once[]=PATH_t3lib.'class.t3lib_pagetree.php';
	...
	function mailModule_main()	{
		...
		*/
		...
		switch ($this->CMD) {
			...
			case 'send_mail_test':
				//send test mail
				$theOutput.= $this->showSteps((3-(5-$totalSteps)),$totalSteps);
				...
				$theOutput.= '<input type="hidden" name="CMD" value="cats">';
				$theOutput.= '<input type="hidden" name="sys_dmail_uid" value="'.$this->sys_dmail_uid.'">';
				$theOutput.= '<input type="hidden" name="pages_uid" value="'.t3lib_div::_GP('pages_uid').'">';
				$theOutput.= '<input type="hidden" name="currentCMD" value="'.$this->CMD.'">';
				$theOutput.= '<input class="next" type="submit" value="'.$LANG->getLL('dmail_wiz_next').'">';
			...
			case 'cats':
				...
				// content of step
				$theOutput.= '<br /><div id="box-1" class="toggleBox">';
				$theOutput.= $this->createCategoriesForm();
				$theOutput.= '</div></div>';
				// Title
				$theOutput = $this->doc->section('Categories',$theOutput,1,1,0, TRUE);

				$theOutput.= '<input type="hidden" name="CMD" value="send_mass">';
				$theOutput.= '<input type="hidden" name="sys_dmail_uid" value="'.$this->sys_dmail_uid.'">';
				$theOutput.= '<input type="hidden" name="pages_uid" value="'.t3lib_div::_GP('pages_uid').'">';
				$theOutput.= '<input type="hidden" name="currentCMD" value="'.$this->CMD.'">';
				$theOutput.= '<input type="hidden" name="test" value="zzzzANCyyy">';
				$theOutput.= '<input class="next" type="submit" value="'.$LANG->getLL('dmail_wiz_next').'">';
				$theOutput.= '<input type="submit" class="back" value="'.$LANG->getLL('dmail_wiz_back').'" name="back">';
				break;
			...
			case 'send_mail_final':
			case 'send_mass':
				if(t3lib_div::_GP('CRL')==$LANG->getLL('dm_02')){
					$theOutput.= $this->createReceipientList();
				}
	...
	function cmd_finalmail($row)	{
		...
		while($row = $TYPO3_DB->sql_fetch_assoc($res))	{
			$select = '';
			if ($row['uid']==$this->receipientlist) {$select='selected="selected"';}
			$opt[] = '<option value="'.$row['uid'].'" '.$select.'>'.htmlspecialchars($row['title']).'</option>';
		}
	...
	/**
	 * create a new resipient list
	 * @param	array		.
	 *
	 * @return void
	 */
	function createReceipientList(){
		global $BACK_PATH, $TYPO3_DB, $LANG;

		$data = array(
			'pid' => $this->pageinfo['uid'],
			'tstamp' => time(),
			'title' => t3lib_div::_GP('rlName'),
			'description' => t3lib_div::_GP('rlDesc')
		);

		$create = $TYPO3_DB->exec_INSERTquery('sys_dmail_group', $data);
		$whereClause = "title like '".$data['title']."'";
		$res = $TYPO3_DB->exec_SELECTquery('uid', 'sys_dmail_group', $whereClause, '', '');
		$row = $TYPO3_DB->sql_fetch_assoc($res);
		$data['uid'] = $row['uid'];
		$this->receipientlist=$data['uid'];

		$out = "";
		$whereClause = '';
		// 'user_feuserextension_order'
		$res = $TYPO3_DB->exec_SELECTquery('*', 'sys_dmail_category', $whereClause, '', 'user_feuserextension_order');

		while($row = $TYPO3_DB->sql_fetch_assoc($res))	{
			$rowd = array(
				'uid_local' => $data['uid'],
				'uid_foreign' => t3lib_div::_GP(urlencode($row['category']))
			);
			$create = $TYPO3_DB->exec_INSERTquery('sys_dmail_group_category_mm', $rowd);
		};
	...
	/**
	 * show the direct mail categories
	 * @param	array		$fields, $table, $whereClause, $press, $order, $style
	 *
	 * @return	string		HTML list showing the categories
	 */
		function createCategoriesList($fields, $table, $whereClause, $press, $order, $style) {
			...
			while($row = $TYPO3_DB->sql_fetch_assoc($res))	{
				...
				$whereClause = 'user_feuserextension_parent='.$row['uid'];
				$whereClause.=' and user_feuserextension_press='.$press;
				$res2 = $TYPO3_DB->exec_SELECTquery('*', 'sys_dmail_category', $whereClause, '', 'user_feuserextension_order');
				if ($res2) $out.='<ul style="list-style:none">';
				while($row2 = $TYPO3_DB->sql_fetch_assoc($res2))	{
						$out.='<li><input style="margin-right: 5px;margin-top: 3px;" type="checkbox" name="'.urlencode($row2['category']).'" value="'.$row2['uid'].'" title="'.$row2['user_feuserextension_description'].'">'.$row2['category'].'</input></li>';
				};
				if ($res2) $out.='</ul>';
				$out.='</li>';
			};
	...
	function createCategoriesForm() {
		...
		$out.='<input name="CRL" value="'.$LANG->getLL('dm_03').'" type="submit">';
		//$out.=t3lib_div::view_array($this->params);
		...
		if (!$TYPO3_DB->sql_num_rows($res))	{
			...
			while($row = $TYPO3_DB->sql_fetch_assoc($res))	{
				$outLines[] = array(
					...
					'<a href="#" onClick="'.t3lib_BEfunc::viewOnClick($row['uid'],$BACK_PATH,'','','',$this->implodedParams['HTMLParams']).'"><img src="../images/preview_html.gif" width="16" height="16" alt="" style="vertical-align:top;" title="'.$LANG->getLL('nl_viewPage_HTML').'"/></a>',
					'<a href="#" onClick="'.t3lib_BEfunc::viewOnClick($row['uid'],$BACK_PATH,'','','',$this->implodedParams['plainParams']).'"><img src="../images/preview_txt.gif" width="16" height="16" alt="" style="vertical-align:top;" title="'.$LANG->getLL('nl_viewPage_TXT').'"/></a>',
					);
			}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tx_dm_newsletter/mod1/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tx_dm_newsletter/mod1/index.php']);
}
